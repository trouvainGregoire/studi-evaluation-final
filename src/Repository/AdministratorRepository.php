...
class AdministratorRepository extends ServiceEntityRepository implements PasswordUpgraderInterface
{
     ...
     */
    public function upgradePassword(UserInterface $administrator, string $newEncodedPassword): void
    {
        if (!$administrator instanceof Administrator) {
            throw new UnsupportedUserException(sprintf('Instances of "%s" are not supported.', \get_class($administrator)));
        }


        $administrator->setPassword($newEncodedPassword);
